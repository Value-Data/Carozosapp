...
from .helpers import (
    norm, norm_cols, pct_to_fraction, normalize_bounds, 
    parse_calibre_cols, pick_col
)
...
def process_asignacion(lotes_df, tolerancias_df, disminucion_df, cruce_df, especie=None, linea_producto=None):
    """
    ...
    cols_500_600 = [c for c in lotes_adj.columns 
                    if str(c).startswith("500.0__") or str(c).startswith("600.0__")]
    ...
    pct_col = "% DISMINUCION" if "% DISMINUCION" in disminucion_df.columns else disminucion_df.columns[1]
    ...
    for key in join_keys:
        if key not in lotes_adj.columns:
            raise ValueError(f"Columna '{key}' no encontrada en lotes. Columnas disponibles: {list(lotes_adj.columns)}")
        if key not in tolerancias_df.columns:
            raise ValueError(f"Columna '{key}' no encontrada en tolerancias. Columnas disponibles: {list(tolerancias_df.columns)}")
    
    cand = lotes_adj.merge(tolerancias_df, on=join_keys, how="inner", suffixes=("", "_TOL"))
    
    if len(cand) == 0:
        raise ValueError(f"No se encontraron coincidencias entre lotes y tolerancias para la combinación especificada. "
    ...
    if len(detalle) == 0:
        raise ValueError("No se generaron filas en el procesamiento. Verifique que haya coincidencias entre lotes y tolerancias.")
    ...
    if "MERCADO-CLIENTE" not in detalle.columns and "MERCADO_CLIENTE" not in detalle.columns:
        ...
        try:
            col_mc_detalle = pick_col(detalle, ["MERCADO-CLIENTE", "MERCADO_CLIENTE", "MERCADOCLIENTE"])
        except KeyError:
            raise ValueError(f"Columna MERCADO-CLIENTE no encontrada. Columnas disponibles: {list(detalle.columns)}")
    else:
        col_mc_detalle = "MERCADO-CLIENTE" if "MERCADO-CLIENTE" in detalle.columns else "MERCADO_CLIENTE"
    
    res_mc = (detalle
              .groupby(col_mc_detalle, as_index=False)
