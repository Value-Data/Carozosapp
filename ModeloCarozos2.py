...
nect_adj = nect.copy()
cols_500_600 = [
    c for c in nect_adj.columns if str(c).startswith("500.0__") or str(c).startswith("600.0__")
]
...
# ========= RESÚMENES =========
res_mc = (
    detalle.groupby("MERCADO-CLIENTE", as_index=False)
...
def build_check_sheet_for_lote(lote_id: int) -> pd.DataFrame:
    ...
    return pd.DataFrame(
        rows_ck,
        columns=["Variable", "Valor Real", "%Disminucion", "Valor Disminuido"],
    )
...
with pd.ExcelWriter(OUT_XLSX, engine=writer_engine) as xw:
    ...
    if CHECK_LOTE is not None:
        build_check_sheet_for_lote(CHECK_LOTE).to_excel(
            xw,
            index=False,
            sheet_name=f"Check_{CHECK_LOTE}",
        )
